...
def process_events(event_stream):
    ...
    while True:
        try:
            ...
            if player not in printed_players and len(printed_players) <= 10:
                print(
                    f"Event {event_id}: Player {player} "
                    f"(level {level}) {action}"
                )
                printed_players.add(player)
...
def main():
    ...
    total, high_level, treasure, level_up = process_events(stream)
